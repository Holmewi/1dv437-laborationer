...
{
	public class SplitterParticle : Model.Particle
	{
		private Random random;

		private const float MAX_LIFE = 1.0f;
		private const float MAX_SPEED = 0.1f;
		private const float MAX_SIZE = 0.03f;
		private const float MIN_SIZE = 0.01f;

		private float timeLived = 0;
		private float lifePercent;

		public SplitterParticle(Texture2D texture, Vector2 position)
		{
			this.random = new Random ();

			base.Texture = texture;
			base.Position = position;
			base.Acceleration = new Vector2((float)this.random.NextDouble() * (0.5f - -0.5f) + -0.5f, (float)this.random.NextDouble() * (0.5f - -0.5f) + -0.5f);
			base.Gravity = new Vector2(0.5f, 0.5f);
